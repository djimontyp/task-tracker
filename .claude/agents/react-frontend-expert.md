...
- ✅ **Якщо потрібен backend контекст** (API endpoints, request/response schemas):
  - Status: Blocked, Domain: backend, Required: "API contract for /auth/login endpoint"
  - Coordinator делегує до backend expert, ти отримаєш контекст через resume та збережеш всю попередню роботу
