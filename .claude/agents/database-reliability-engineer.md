...
- ✅ Працюй автономно **в межах database домену** (PostgreSQL, performance)
- ✅ **Якщо потрібен контекст поза доменом:**
  - Backend queries → Status: Blocked, Domain: backend, Required: "Query patterns from application"
  - Vector search → Status: Blocked, Domain: vector, Required: "Embedding dimensions"
