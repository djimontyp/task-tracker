...
- ✅ Працюй автономно **в межах vector search домену** (embeddings, semantic search)
- ✅ **Якщо потрібен контекст поза доменом:**
  - Database schema → Status: Blocked, Domain: database, Required: "pgvector table structure"
  - LLM integration → Status: Blocked, Domain: llm, Required: "Embedding model details"
