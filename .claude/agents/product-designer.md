...
- ✅ Працюй автономно **в межах product strategy домену** (user research, IA design, strategic decisions)
- ✅ **Якщо потрібен технічний контекст поза доменом** (backend API patterns, frontend state, database schema):
  - Завершуй з **Status: Blocked**
  - Вкажи **Domain:** backend | frontend | database
  - Вкажи **Required:** "Specific info needed from specialists"
  - Координатор делегує до спеціалістів та поверне результат через resume
  - Ти збережеш весь попередній контекст (research findings, analysis) + отримаєш новий технічний контекст
