...
class DashboardMetricsResponse(BaseModel):
    ...
    topicQualityScore: float = Field(..., description="Average topic quality score (0-100)", ge=0, le=100)
    noiseRatio: float = Field(..., description="Percentage of messages filtered as noise", ge=0, le=100)
    classificationAccuracy: float = Field(..., description="Classification model accuracy (0-100)", ge=0, le=100)
    activeAnalysisRuns: int = Field(..., description="Number of currently active analysis runs", ge=0)
    trends: dict[str, MetricTrend] = Field(..., description="Trend data for each metric")
