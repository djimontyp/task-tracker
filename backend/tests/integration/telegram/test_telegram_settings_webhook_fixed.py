"""
Fixed comprehensive tests for Telegram webhook manager using proper httpx mocking.
...
class TestTelegramWebhookManagerFixed:
    """Test suite for TelegramWebhookManager class with proper mocking"""
    ...
    async def test_validate_bot_token_success(self, webhook_manager):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            # Set up the mock client get method
    ...
    async def test_validate_bot_token_unauthorized(self, webhook_manager):
        """Test bot token validation with unauthorized response"""
        with patch("httpx.AsyncClient") as mock_client_class:
            mock_client = AsyncMock()
            mock_client_class.return_value.__aenter__.return_value = mock_client
            mock_client_class.return_value.__aexit__.return_value = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_setup_webhook_timeout(self):
        """Test webhook setup timeout"""
        webhook_manager = TelegramWebhookManager(timeout=0.1)
...
class TestTelegramWebhookManagerPerformance:
    """Performance tests for webhook manager"""

    @pytest.mark.asyncio
    async def test_timeout_scenarios(self):
        """Test various timeout scenarios"""
        webhook_manager = TelegramWebhookManager(timeout=0.01)  # Very short timeout
