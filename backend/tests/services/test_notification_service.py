...
def mock_versioning_service():
    ...
    with patch.object(
        notification_service, "versioning_service", spec=VersioningService
    ) as mock:
        yield mock
...
class TestCheckAndSendAlerts:
    ...
    @pytest.mark.asyncio
    async def test_alert_sent_when_threshold_exceeded(
        self, mock_versioning_service, notification_pref
    ):
        ...
        with patch(
            ...
            "app.services.notification_service.telegram_service"
        ) as mock_telegram:
    ...
    @pytest.mark.asyncio
    async def test_email_only_when_telegram_disabled(
        self, mock_versioning_service, notification_pref
    ):
...
class TestSendDailyDigest:
    ...
    @pytest.mark.asyncio
    async def test_digest_sent_when_enabled(
        self, mock_versioning_service, notification_pref
    ):
        ...
        with patch(
            ...
            mock_email.send_daily_digest.assert_called_once_with(
                "test@example.com", stats
            )
            mock_telegram.send_daily_digest.assert_called_once_with("123456789", stats)

    @pytest.mark.asyncio
    async def test_no_digest_when_disabled(
        self, mock_versioning_service, notification_pref
    ):
