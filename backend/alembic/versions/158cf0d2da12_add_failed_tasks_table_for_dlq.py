...
def upgrade() -> None:
    ...
    # Note: This migration was auto-generated but attempts to modify tables
    # that are created by SQLModel models, not migrations.
    # Commented out to prevent errors in fresh installations.

    # Index operations for HNSW vector indexes
    op.execute("DROP INDEX IF EXISTS idx_atoms_embedding_hnsw")
    op.execute("DROP INDEX IF EXISTS idx_messages_embedding_hnsw")

    # Skip index operations on tables that don't exist in migrations:
    # - failed_tasks (created by SQLModel model)
    # - task_execution_logs (created by SQLModel model)
    # These indexes will be created automatically by SQLModel when tables are created.
