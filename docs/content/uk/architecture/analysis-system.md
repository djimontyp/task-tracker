...
!!! abstract "AI-дозовлена класифікація задач"
    Система аналізу трансформує неструктуровані повідомлення в структуровані пропозиції задач за допомогою LLM та налаштовуваних схем класифікації.

**Ключові особливості:**

- Управління життєвим циклом з 7 станами
- Підтримка кількох LLM провайдерів
- Контекст, розширений за допомогою RAG
- Автоматичне розрахування оцінок довіри
- Затвердження за участю людини

### Архітектура високого рівня

```mermaid
stateDiagram-v2
    [*] --> PENDING: Створити запуск
    PENDING --> RUNNING: Розпочати обробку

    RUNNING --> Prefilter: Видалити дублікати
    Prefilter --> Batch: Створити пакети
    Batch --> Embeddings: Генерувати при RAG
    Embeddings --> Context: Побудувати контекст RAG
    Context --> LLM: Класифікувати за допомогою AI
    LLM --> COMPLETED: Усі пакети оброблено

    RUNNING --> FAILED: Сталася помилка
    FAILED --> RUNNING: Повтор

    COMPLETED --> REVIEW_REQUIRED: Низька впевненість
    COMPLETED --> APPROVED: Висока впевненість

    REVIEW_REQUIRED --> APPROVED: Людський перегляд
    APPROVED --> CLOSED: Архівувати
...
AI-згенерована пропозиція з аналізу повідомлень.
...
```python
{
    ...
    "reasoning": "Кілька звітів про iOS crash при вході",
    "llm_metadata": {
        "model": "gpt-4",
        "tokens_used": 2000,
