...
export function MessageAnalysisSection({ message, hasAnalysis, reasoning }: MessageAnalysisSectionProps) {
    ...
    useEffect(() => {
        setLoading(true);
        agentService.listAgents({ active_only: true })
            .then(data => {
                setAgents(data);
                if (data.length > 0) {
                    // Prefer 'knowledge_extractor' if available, otherwise first one
                    const defaultAgent = data.find(a => a.name === 'knowledge_extractor') || data[0];
            ...
            {isRunning && (
                <div className="bg-background/50 rounded-lg p-4 border border-border/40 space-y-3">
                    <div className="flex items-center justify-between text-xs font-medium text-muted-foreground">
                        <span>{t('analysis.processing', 'Processing...')}</span>
                        <span className="animate-pulse text-primary">Running ({progress.topics_created + progress.atoms_created} items)</span>
                    </div>
